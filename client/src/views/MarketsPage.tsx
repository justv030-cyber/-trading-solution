...
// import { calcPnL } from "@/context/PaperTradeContext"; 
...
export function MarketsPage() {
  ...
  const getMarkPrice = (pair: string) => {
    const [base, quote] = pair.split("-");

    const market = overview?.markets?.find(
      m => m.base === base && m.quote === quote
    );
    console.log(market?.mark_price);

    return market?.mark_price ?? 0;


  };

  const calcPnL = (pos) => {
    const mark = getMarkPrice(pos.pair);
    if (pos.side === "long") {
      return (mark - pos.entryPrice) * pos.sizeBase;
    } else {
      return (pos.entryPrice - mark) * pos.sizeBase;
    }
  };
                  ...
                  <div key={i}
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white/50">
                        Entry: {pos.entryPrice} | Size: {pos.sizeBase}
                      </p>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm font-bold"
                        style={{ color: pnl >= 0 ? "#3dffa0" : "#ef5350" }}>
