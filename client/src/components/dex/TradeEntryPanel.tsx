...
export function TradeEntryPanel({ marketId, pair, base, quote, markPrice, connected, onRequestConnect }: Props) {
  ...
  async function onSubmit() {
    console.log("POSTING TO /api/v1/orders");
    setToast(null);
    ...
    if (orderType === "market") {
      if (trade) {
        setToast(`Filled @ ${trade.price}`);
      } else {
        setToast("Market order executed");
      }
    } else {
      setToast(`Limit order ${order.status}`);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-4 border-l border-white/[0.06] bg-[#121519] p-3">
      ...
      <div className="flex gap-1.5">
        {(["market", "limit", "stop"] as const).map((t) => (
          <button key={t} type="button" disabled={t === "stop"}
            ...
            className={`flex-1 rounded border py-1.5 text-xs capitalize transition-colors disabled:opacity-35 ${orderType === t ? "border-[#00d9c0] text-[#00d9c0]" : "border-white/[0.06] bg-[#060809] text-white/45"
              }`}>
            {t}
          </button>
        ))}
      ...
      <button type="button" disabled={!markPrice} onClick={onSubmit}
        className={`w-full rounded py-2.5 text-sm font-bold transition-colors disabled:cursor-not-allowed disabled:opacity-45 ${side === "buy" ? "bg-[#3dd68c] text-black" : "bg-[#ff5c5c] text-white"
          }`}>
