...
// function seedOrderBook(engine) {
//   const now = Math.floor(Date.now() / 1000);

//   // prevent double seeding
//   if (engine.inner?.seeded) return;

//   engine.submit("btc-usdt", "sell", "limit", 66560, 5, now);
//   engine.submit("btc-usdt", "sell", "limit", 66550, 5, now);
//   engine.submit("btc-usdt", "sell", "limit", 66540, 5, now);

//   engine.submit("btc-usdt", "buy", "limit", 66500, 5, now);
//   engine.submit("btc-usdt", "buy", "limit", 66490, 5, now);

//   engine.inner.seeded = true;
// }

if (!globalThis._matchingEngine) {
  globalThis._matchingEngine = createMatchingEngine(matchingPath);
  // seedOrderBook(globalThis._matchingEngine);
