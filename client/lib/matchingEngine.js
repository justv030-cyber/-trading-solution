...
export class MatchingEngine {
    ...
    try {
      if (fsSync.existsSync(this.path)) {
        ...
        if (s.trim()) {
          ...
          console.warn("matching data file empty, starting fresh");
          this.inner = innerEmpty();
        }
      }
    } catch (e) {
  ...
  async submit(marketId, side, orderType, price, size, now) {
    console.log(
      "BOOK EXISTS:",
      this.inner.books.has(marketId)
    );

    console.log(
      "BOOK DATA:",
      this.inner.books.get(marketId)
    );
    ...
    console.log(
      "BOOK:",
      JSON.stringify(this.inner.books.get(marketId), null, 2)
    );
